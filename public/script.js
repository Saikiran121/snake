...
function drawInitialGrid() { clearCanvas(); drawGrid(); }
...
function drawSnake() {
    snake.forEach((segment, index) => {
        ...
        const x = segment.x * gridSize; const y = segment.y * gridSize;
        ...
        if (isHead) {
            ...
            let e1x, e1y, e2x, e2y;
            const eyeSize = 3; const offset = 4;
            
            e1x = x + size/2 - offset; e1y = y + offset + 2;
            e2x = x + size/2 + offset; e2y = y + offset + 2;
            
            if (dx === 1) {
                e1x = x + size - offset - 2; e1y = y + size/2 - offset;
                e2x = x + size - offset - 2; e2y = y + size/2 + offset;
            } else if (dx === -1) {
                e1x = x + offset + 2; e1y = y + size/2 - offset;
                e2x = x + offset + 2; e2y = y + size/2 + offset;
            } else if (dy === 1) {
                e1x = x + size/2 - offset; e1y = y + size - offset - 2;
                e2x = x + size/2 + offset; e2y = y + size - offset - 2;
            }
...
function drawFood() {
    const x = food.x * gridSize; const y = food.y * gridSize; const center = gridSize / 2;
    
    ctx.shadowBlur = 25; ctx.shadowColor = colors.foodGlow; ctx.fillStyle = colors.food;
    ctx.beginPath(); ctx.arc(x + center, y + center, gridSize/2 - 2, 0, Math.PI * 2); ctx.fill();
    
    ctx.shadowBlur = 0; ctx.fillStyle = '#fff';
    ctx.beginPath(); ctx.arc(x + center - 2, y + center - 2, 3, 0, Math.PI * 2); ctx.fill();
}
...
function handleKeyPress(event) {
    ...
    const isGoingUp = dy === -1; const isGoingDown = dy === 1;
    const isGoingRight = dx === 1; const isGoingLeft = dx === -1;
    
    if (leftKeys.includes(event.key) && !isGoingRight) {
        dx = -1; dy = 0; changingDirection = true; highlightKey(2); event.preventDefault();
    } else if (upKeys.includes(event.key) && !isGoingDown) {
        dx = 0; dy = -1; changingDirection = true; highlightKey(0); event.preventDefault();
    } else if (rightKeys.includes(event.key) && !isGoingLeft) {
        dx = 1; dy = 0; changingDirection = true; highlightKey(3); event.preventDefault();
    } else if (downKeys.includes(event.key) && !isGoingUp) {
        dx = 0; dy = 1; changingDirection = true; highlightKey(1); event.preventDefault();
    }
